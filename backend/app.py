...
Risk logic combines sensor thresholds AND the latest YOLO person flag.
"""
...
def init_db():
    ...
    """)
    # migration: add distance_m if the table already existed from before this change
    try:
        conn.execute("ALTER TABLE life_points ADD COLUMN distance_m REAL")
...
GAS_WARNING = 400
GAS_CRITICAL = 700
DISTANCE_CRITICAL_CM = 15  # something is very close
...
def person():
    ...
    with state_lock:
        ...
        latest_state["person_distance_m"] = distance_m
        latest_state["person_status"] = person_status(latest_state)
    ...
    try:
        conn.execute(
            """INSERT OR IGNORE INTO life_points
               (label, lat, lon, confidence, distance_m, timestamp)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (label, lat, lon, data.get("confidence"), data.get("distance_m"), data.get("timestamp", time.time())),
        )
        conn.commit()
    finally:
        conn.close()
